...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	officialBody, err := sanitizeOpenAIResponsesOfficialRequestBody(body)
	if err != nil {
		return nil, err
	}
	body = officialBody
...
func writeOpenAIPassthroughErrorHeaders(dst, src http.Header) {
	...
	rawRetryAfter := src.Get("Retry-After")
	if validOpenAIPassthroughRetryAfter(rawRetryAfter, time.Now()) {
		dst.Set("Retry-After", strings.TrimSpace(rawRetryAfter))
	}
}

func validOpenAIPassthroughRetryAfter(raw string, now time.Time) bool {
	return isSafeRetryAfterAt(raw, now)
}

const maxSafeRetryAfter = 7 * 24 * time.Hour

// IsSafeRetryAfter accepts bounded delay-seconds and future HTTP dates for
// downstream retry hints. It rejects untrusted header content and excessive
// backoff values that could stall clients indefinitely.
func IsSafeRetryAfter(raw string) bool {
	return isSafeRetryAfterAt(raw, time.Now())
}

func isSafeRetryAfterAt(raw string, now time.Time) bool {
	...
	if delaySeconds {
		...
		return err == nil && seconds > 0 && seconds <= uint64(maxSafeRetryAfter/time.Second)
	}
	parsed, err := http.ParseTime(raw)
	return err == nil && parsed.After(now) && !parsed.After(now.Add(maxSafeRetryAfter))
}
