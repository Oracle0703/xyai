...
func (s *OpenAIGatewayService) forwardResponsesViaRawChatCompletions(
	...
	options := openAICompatibleResponsesToChatOptions(account, upstreamModel)
	options.ReasoningContentByID = s.reasoningContentByID
	chatReq, err := apicompat.ResponsesToChatCompletionsRequestWithOptions(&responsesReq, options)
	if err != nil {
		writeOpenAIResponsesFallbackError(c, http.StatusBadRequest, "invalid_request_error", err.Error())
...
func openAICompatibleResponsesToChatOptions(account *Account, upstreamModel string) *apicompat.ResponsesToChatOptions {
	return &apicompat.ResponsesToChatOptions{
