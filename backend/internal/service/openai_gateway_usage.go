...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if err != nil {
		...
		cost = openAIUsageZeroCostBreakdown(result)
	}
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	if result != nil && result.ImageCount > 0 {
		...
		if openAIResolvedTokenPricingUnavailableForImage(resolved) {
			return nil, fmt.Errorf("no pricing available for model: %s: %w", billingModel, ErrModelPricingUnavailable)
		}
	}
...
func openAIUsageZeroCostBreakdown(result *OpenAIForwardResult) *CostBreakdown {
	billingMode := string(BillingModeToken)
	if result != nil && result.ImageCount > 0 {
		billingMode = string(BillingModeImage)
	}
...
func openAIResolvedTokenPricingUnavailableForImage(resolved *ResolvedPricing) bool {
	if resolved == nil || resolved.Mode != BillingModeToken || len(resolved.Intervals) > 0 {
		return false
	}
	if pricing := resolved.BasePricing; pricing != nil && openAIModelPricingHasAnyTokenPrice(pricing) {
		return false
	}
	if chPricing := resolved.channelPricing; chPricing != nil && openAIChannelPricingHasAnyTokenPrice(chPricing) {
		return false
	}
	return true
}

func openAIModelPricingHasAnyTokenPrice(pricing *ModelPricing) bool {
	if pricing == nil {
		return false
	}
...
func openAIChannelPricingHasAnyTokenPrice(pricing *ChannelModelPricing) bool {
	if pricing == nil {
		return false
	}
	if pricing.InputPrice != nil || pricing.OutputPrice != nil || pricing.CacheWritePrice != nil ||
		pricing.CacheReadPrice != nil || pricing.ImageOutputPrice != nil {
		return true
	}
