...
func runtimeCacheTestPromptRiskConfig(t *testing.T, keyword string) string {
	t.Helper()
	cfg := DefaultPromptRiskConfig()
	cfg.Enabled = true
	cfg.Mode = PromptRiskModeBlock
	cfg.AllGroups = true
	cfg.KeywordSets = []PromptRiskKeywordSet{{
		Level:     PromptRiskLevelHigh,
		MatchMode: PromptRiskMatchContains,
		Score:     defaultPromptRiskHighScore,
...
func TestContentModerationRuntimeSnapshotRiskControlSettingUpdateIsImmediate(t *testing.T) {
	repo := &contentModerationRuntimeSettingRepo{values: map[string]string{
	...
	require.NoError(t, err)
	require.False(t, snapshot.riskControlEnabled)

	err = settingService.UpdateSettings(context.Background(), &SystemSettings{RiskControlEnabled: true})
	require.NoError(t, err)
	...
	err = settingService.UpdateSettings(context.Background(), &SystemSettings{RiskControlEnabled: false})
	require.NoError(t, err)
	snapshot, err = moderationService.loadRuntimeSnapshot(context.Background())
	require.NoError(t, err)
	require.False(t, snapshot.riskControlEnabled)
...
func TestContentModerationRuntimeSnapshotUpdatePromptRiskConfigIsImmediate(t *testing.T) {
	repo := &contentModerationRuntimeSettingRepo{values: map[string]string{
		SettingKeyRiskControlEnabled:      "true",
		SettingKeyContentModerationConfig: runtimeCacheTestConfig(t),
		SettingKeyPromptRiskConfig:        runtimeCacheTestPromptRiskConfig(t, "old-risk"),
	}}
	svc := runtimeCacheTestService(repo, time.Hour)

	decision, err := svc.Check(context.Background(), runtimeCacheTestInput("new-risk"))
	require.NoError(t, err)
	require.True(t, decision.Allowed)

	updated := DefaultPromptRiskConfig()
	updated.Enabled = true
	updated.Mode = PromptRiskModeBlock
	updated.AllGroups = true
	updated.KeywordSets = []PromptRiskKeywordSet{{
		Level:     PromptRiskLevelHigh,
		MatchMode: PromptRiskMatchContains,
		Score:     defaultPromptRiskHighScore,
		Keywords:  []string{"new-risk"},
	}}
	_, err = svc.UpdatePromptRiskConfig(context.Background(), updated)
	require.NoError(t, err)

	decision, err = svc.Check(context.Background(), runtimeCacheTestInput("new-risk"))
	require.NoError(t, err)
	require.True(t, decision.Blocked)
	decision, err = svc.Check(context.Background(), runtimeCacheTestInput("old-risk"))
	require.NoError(t, err)
	require.True(t, decision.Allowed)

	getValue, getMultiple := repo.calls()
	require.Equal(t, 1, getValue)
	require.Equal(t, 1, getMultiple)
}
