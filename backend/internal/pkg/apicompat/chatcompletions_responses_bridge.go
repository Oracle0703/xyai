...
type ResponsesToChatOptions struct {
	...
	// DropTemperature omits temperature for compatible upstreams that reject it.
	DropTemperature bool
	// DropMaxCompletionTokens omits max_completion_tokens for compatible upstreams that reject it.
...
func ResponsesToChatCompletionsRequestWithOptions(req *ResponsesRequest, opts *ResponsesToChatOptions) (*ChatCompletionsRequest, error) {
	...
	if opts == nil || !opts.DropMaxCompletionTokens {
		out.MaxCompletionTokens = req.MaxOutputTokens
	}
	if opts == nil || !opts.DropTemperature {
