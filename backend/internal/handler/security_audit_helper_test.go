...
func TestBuildSecurityAuditRequestPreservesPromptRiskJudgeHeader(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", nil)
	c.Request.Header.Set(service.PromptRiskJudgeHeaderName, "signed-judge-request")

	request := buildSecurityAuditRequest(
		c,
		nil,
	...
	require.Equal(t, "signed-judge-request", request.PromptRiskJudgeHeader)
}
