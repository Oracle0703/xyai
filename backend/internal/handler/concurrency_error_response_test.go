...
func TestConcurrencyErrorResponse(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "concurrency cache error keeps explicit service message",
			err:         service.NewConcurrencyCacheError("user", errors.New("redis unavailable")),
			slotType:    "user",
			wantStatus:  http.StatusServiceUnavailable,
			wantType:    "server_error",
			wantMessage: "Concurrency service unavailable, please retry later",
		},
		{
			name:        "generic redis acquire error is service unavailable",
