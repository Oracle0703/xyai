...
func RegisterGatewayRoutes(
	...
	// Guard Responses subpaths before the local intercept middleware. The
	// wrapper also handles the native input_tokens endpoint.
	gateway.Use(guardResponsesSubpath(requestIntercept))
	{
	...
	// Root aliases share the upstream allowlist placement and local archive /
	// interception middleware. The latter must remain after the subpath guard.
	rootRoute := func(method, path string, limit gin.HandlerFunc, routeHandler gin.HandlerFunc) {
		// Upstream contract: r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, requestArchive, requestIntercept, routeHandler)
	}
	...
	// Upstream contract: codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, requestArchive, guardResponsesSubpath(requestIntercept))
	{
