...
- `POST /api/v1/admin/subscriptions/reset-daily-filtered` 在单条 PostgreSQL 语句内选择并更新匹配订阅；无论请求是否带 status, 写入时都只处理 active、未删除、未过期且用户/分组未删除的订阅。组织按当前用户邮箱域名精确筛选 `xunyou.com` / `wsdashi.com`, 忽略大小写但不包含子域名。Ent 的 PostgreSQL 组织筛选谓词必须用 `entsql.P` 配合 `Builder.Arg` 延迟生成占位符, 不能把带 `?` 的 `ExprP` 原始表达式直接嵌入 EXISTS 子查询。
