...
- 2026-08-26 在 `feature/hy/10183_merge_sub2api_183` 以最新 `main@0747c896abf8202dff9935a30f020317ebdc91fc` 合并 `Wei-Shaw/sub2api main@7634e3c23b5b9afc588c37b170820f63f1d41bbb`，merge base 为上游 `baeac1f3de21d37b129405f092ef86c24b3f203d`，当前后端版本 `0.1.183`，merge commit 待用户审核后创建。固定目标包含 0.1.178-0.1.183 的 410 commits、771 paths、`+71087/-5334`，引入 Kimi/智谱/DeepSeek 与 adaptive protocol、Channel Monitor 配额模式、渠道分时及 Fast/Flex 倍率、OpenAI OAuth transport 插件、Responses input-tokens 和大量网关兼容加固；Go 基线升到 `1.27.0`。20 个文本冲突按语义解决：Wire 同时保留本地能力与上游 CN provider/Plugin/Model Plaza；Responses->Chat 和 Tool Schema 的重叠部分采用上游主实现，仅保留本地独有 options/schema 清洗；路由继续保证 Responses guard 位于 `RequestIntercept` 前。本地 24 个 `docs/features` 文件零删除，独有的 RequestArchive/RequestIntercept、Prompt Metrics/Risk、Token Analysis、组织用量、子管理员、compatible cache usage、默认 reasoning effort、并发 preset、quota flusher 和 reasoning-only failover 均保留；仅记录、不修复上游独有问题。
