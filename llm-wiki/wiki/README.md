...
- 2026-08-03 从本地 `main@12770bc5da8c0ef6a2dd02b0e80a37f6eb26d408` 创建 `feature/hy/10170_merge_upstream_v170`, 固定合入 `Wei-Shaw/sub2api main@7e2e9ba05026b7126318aa0754c1afa0ac00bc58`, merge base 为 `5a6143097db142b72a6fc848c214e97214470bdd`, 当前后端版本 `0.1.170`; merge commit 待用户审核后创建。上游增量为 99 commits、294 files、`+16749/-1179`, 增加分组利润控制、跨平台 API Key 上游账单探测与倍率同步、账号跨页全选/批量删除、内容审核代理、上游 path allowlist、compact home、订阅配额窗口到期对齐和部署加固。6 个文本冲突按语义并集解决：双方独有能力并存, 重叠实现采用上游语义；Wire 补齐 `ProxyRepository`, 本地测试 adapter 传入 `nil`, Responses path guard 位于 `RequestIntercept` 前。23 个 tracked `docs/features/` 文件零删除。本地 RequestArchive/RequestIntercept、Prompt Metrics/Risk 与 LLM judge、Token Analysis、组织用量、子管理员、compatible cache usage、默认 reasoning effort、用户并发 preset 和 quota flusher 继续保留。最终验证中 default/integration Go、目标代码生成、server build、前端 lint/typecheck/build 和 deploy fixtures 退出 0；unit tag 仅有第一父既有 `/auth/me` fixture mismatch, Go lint 28 项均为第一父既有, 完整 Vitest 仅有 2 条第一父既有 rollback 断言失败和 10 个既有 GroupsView mock unhandled rejection, 按用户约束均未修复。
