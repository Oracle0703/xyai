...
- 2026-08-05 从本地 `main@12770bc5da8c0ef6a2dd02b0e80a37f6eb26d408` 创建 `feature/hy/10171_merge_sub2api_171`, 固定合入 `Wei-Shaw/sub2api main@aac53afe0ef1ae850e2f18b5d2814ac67c835e7e`, 当前后端版本 `0.1.171`; merge commit 待用户审核后创建。上游增量为 440 个文件、`+28233/-2050`, 增加登录/OAuth/Passkey 的腾讯/阿里云动作验证码、分组利润控制及 profit-preview、内容审核代理选择、OpenAI Codex 客户端版本自动同步、上游倍率探测多平台写回、Live capability 管理端确认和 Responses tool-output-media 兼容。11 个文本冲突按语义并集解决；`docs/features/` 本地新增功能文档零删除。本地 RequestArchive/RequestIntercept、Prompt Metrics/Risk 与 LLM judge、Token Analysis、组织用量、子管理员、OpenAI-compatible cache usage、默认 reasoning effort、用户并发 preset 和 quota flusher 继续保留；本分支只解决合并冲突及必要测试 mock 适配, 不修复上游自身问题。
