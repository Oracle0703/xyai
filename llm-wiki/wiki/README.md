...
- 2026-08-17 在 `feature/hy/10177_merge_sub2api_177` 以最新本地 `main@65bd97fe3b8e78888f544f26027081bec97f7177` 合并 `Wei-Shaw/sub2api main@baeac1f3de21d37b129405f092ef86c24b3f203d`, merge base 为上游 `0e82efe48951cb7da1f8554639afdeab05bf16b8`, 当前后端版本 `0.1.177`; 保持 `MERGE_HEAD` 等待用户审核。上游增量为 17 commits、76 paths、`+4522/-326`, 增加按配置时区维护的分组日用量汇总和昨日金额、OpenAI 原生 remote compaction v2、`x-codex-turn-state` 同回合转发与跨账号回放防护，并把 Codex 指纹收敛改为显式 opt-in 且覆盖 passthrough；Go 基线升到 `1.26.6`。本轮 0 个文本冲突，20 个双方修改路径完成三方语义审查；24 个 `docs/features` 文件及本地 RequestArchive/RequestIntercept、Prompt Metrics/Risk、Token Analysis、组织用量、子管理员、compatible cache usage、默认 reasoning effort、并发 preset、quota flusher 和 reasoning-only failover 继续保留，不修复上游自身问题。
