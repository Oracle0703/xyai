...
`POST /api/v1/admin/subscriptions/reset-daily-filtered` 无视全局 `idempotency.observe_only`, 始终强制非空 `Idempotency-Key`; coordinator 或事务能力不可用时 fail-close。scope 为 `admin.subscriptions.reset_daily_filtered`, 规范化后的五个筛选字段参与幂等 payload, 同键同 payload 重放既有 `reset_count`, 同键不同 payload 按通用幂等冲突处理。订阅批量 UPDATE 与幂等 `MarkSucceeded` 在同一 Ent/PostgreSQL 事务内提交, 标记失败会回滚 UPDATE；缓存只在提交成功后失效。执行失败后的 `MarkFailedRetryable` 使用保留 context values、但不继承请求取消信号的独立 2 秒有界 context, 不得复用已经取消的请求 context 而让记录滞留在 `processing`。`Commit` 返回错误且结果不确定时, coordinator 使用独立短超时 context 回读记录；只有 status、request fingerprint、HTTP status 和 stored response 全部与本次执行一致才恢复成功并执行 post-commit callback, 否则继续 fail-close。共享 billing cache 删除与跨实例发布使用固定 worker 和单个总时间预算、独立 context；预算耗尽会记录跳过任务数和有限 cache-key 样本, 未失效缓存依赖既有 TTL 最终收敛。缓存失败不回滚或伪报数据库更新失败。
...
OpenAI Responses `text.format.schema` 只允许官方 Structured Outputs 子集中的 `format`: `date-time`, `time`, `date`, `duration`, `email`, `hostname`, `ipv4`, `ipv6`, `uuid`。`OpenAIGatewayService.Forward` 会在首次上游请求前仅清理 `text.format.schema` 子树里的不支持格式（如 `uri`）, 不改动 `metadata.format` 等业务字段；相关测试入口为 `TestOpenAIGatewayService_StripsUnsupportedTextFormatSchemaFormatBeforeFirstForward`。
