  ...
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        ...
        <div
          v-if="canManageSubscriptions && selectedCount > 0"
      ...
      <template #table>
        <DataTable
          ...
          :loading="loading"
          row-key="id"
          :selectable="canManageSubscriptions"
    ...
    <BulkSubscriptionActionDialog
      v-if="canManageSubscriptions && bulkAction !== null"
      :show="true"
      :action="bulkAction"
...
const openBulkAction = (action: SubscriptionBulkAction) => {
  if (!canManageSubscriptions.value || loading.value || bulkTargets.value[action].length === 0) return
