export default {
  ...
  admin: {
    ...
    promptMetrics: {
      title: 'Prompt 指标',
      description: '查看用户提示词交互, 趋势, 维度排行和质量分析',
      project: '项目',
      branch: '分支',
      client: '客户端',
      onlyLowQuality: '仅低质量',
    ...
    riskControl: {
      ...
      action: {
        block: '拦截',
        keywordBlock: '关键词拦截',
        promptRiskBlock: 'Prompt 风险拦截',
        promptRiskObserve: 'Prompt 风险观察',
        cyberPolicy: '网络安全策略',
        error: '异常',
      },
      promptRisk: {
        intro: '请求送达上游前的本地 Prompt 风险审查，保护团队共享的 OpenAI/Codex 账号不被滥用分类器误判。首版为「观察优先」：只分级记录、不拦截，校准后再把模式切到「拦截」。',
        enabled: '启用 Prompt 风险审查',
        enabledHint: '独立于内容审核 API 阶段，但仍受风控总开关控制。',
        mode: '模式',
        modeHint: '观察：只记录。拦截：高危拒绝并返回改写建议。',
        modeOff: '关闭',
        modeObserve: '观察（只记录）',
        modeBlock: '拦截',
        inputScope: '审查范围',
        inputScopeHint: 'newest:只看最新一轮用户意图(推荐,避免历史轮次重复触发);full:扫描请求体内全部 user turn。',
        scopeNewest: '仅最新一轮',
        scopeFull: '全部上下文',
        allGroups: '全部分组',
        allGroupsHint: '关闭后仅审查下方分组（按分组 opt-in）。',
        groupIds: '分组 ID',
        idsPlaceholder: '逗号分隔，如 1, 2, 3',
        threshold: '升级阈值',
        thresholdHint: '达到该分数升级为拦截。默认 1.0：任一高危词，或 3 个以上不同的中危词。',
        blockStatus: '拦截 HTTP 状态码',
        blockMessage: '拦截提示',
        rewriteSuggestion: '改写建议',
        keywordSets: '关键词集',
        addSet: '新增词集',
        level: '等级',
        matchMode: '匹配模式',
        ...
        levelHigh: '高',
        exemptions: '豁免',
        exemptionsHint: '命中的主体会被封顶到指定等级（记录，但永不硬拦）。',
        addExemption: '新增豁免',
        userIds: '用户 ID',
        apiKeyIds: 'API Key ID',
        maxLevel: '封顶等级',
        save: '保存 Prompt 风险',
        saved: 'Prompt 风险配置已保存',
        ...
        judge: {
          title: 'LLM 语义复核',
          hint: '关键词命中且会真正拦截时，再调一次 LLM 做语义精判，区分“配置自有 VPN/授权测试”与“真实滥用”，降低双用途词误杀。调用失败一律放行（fail-open）。',
          enabled: '启用语义复核',
          baseUrl: 'Base URL（OpenAI 兼容）',
          model: '模型',
          apiKey: 'API Key',
          apiKeyHint: '已配置后留空表示沿用旧 Key；GET 不回传明文。',
          timeoutMs: '超时（毫秒）',
          triggerLevels: '触发等级',
          triggerLevelsHint: '仅对命中这些等级、且会真正拦截的请求做复核（默认仅“高”）。',
          promptTemplate: '自定义复核提示词（system）',
          promptTemplatePlaceholder: '留空使用内置默认提示词',
          recursionTitle: '⚠️ 防止自触发（重要）',
          recursionHint: '复核请求会打到你自己的网关、再次经过本审查，而它含原文关键词，会自触发甚至递归。请为 judge 用一个专属 API Key，并把它的 api_key_id 加入下方豁免（封顶等级=低），同时确保该 Key 所属分组不在内容审核拦截范围内。',
          addExemption: '为 judge 专属 Key 加一条豁免',
          exemptionAdded: '已新增豁免（封顶=低），请填入 judge 专属 api_key_id',
        },
      },
    },
    ...
    settings: {
      ...
      requestArchive: {
        ...
        captureResponse: '捕获响应用量',
        captureResponseHint: '记录响应大小、哈希、流式标记和 token usage，不保存响应正文',
        dir: '归档目录',
        dirHint: '填写绝对路径可把归档切换到其他磁盘；保存时会校验磁盘是否存在、目录可创建且可写，保存后立即生效，无需重启',
        dirDefault: '配置文件默认',
        dirReset: '恢复默认',
