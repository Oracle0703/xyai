export default {
  ...
  admin: {
    ...
    promptMetrics: {
      ...
      description: 'View user prompt interactions, trends, rankings, and quality analysis',
      project: 'Project',
      branch: 'Branch',
      client: 'Client',
      onlyLowQuality: 'Low quality only',
      totalEvents: 'Events',
      activeUsers: 'Users',
      ...
      pending: 'Pending',
      cost: 'Cost',
      avgQuality: 'Avg score',
      trend: 'Interaction trend',
      day: 'Day',
      hour: 'Hour',
      rank: 'Ranking',
      user: 'User',
    ...
    riskControl: {
      ...
      action: {
        ...
        promptRiskBlock: 'Prompt Risk Block',
        promptRiskObserve: 'Prompt Risk Observe',
        cyberPolicy: 'Cyber policy',
        error: 'Error',
      },
      promptRisk: {
        intro: 'Pre-send prompt screening that protects shared upstream OpenAI/Codex accounts from abuse-classifier flags. Ships observe-first: classify and log without blocking until you calibrate, then switch Mode to Block.',
        enabled: 'Enable Prompt Risk',
        enabledHint: 'Independent of the content moderation API stage; still gated by the Risk Control master switch.',
        mode: 'Mode',
        modeHint: 'Observe: log only. Block: refuse high risk and return a rewrite suggestion.',
        modeOff: 'Off',
        modeObserve: 'Observe (log only)',
        modeBlock: 'Block',
        inputScope: 'Input scope',
        inputScopeHint: 'Newest: only the latest user turn (recommended, avoids re-firing on history). Full: scan all user turns in the request.',
        scopeNewest: 'Newest turn only',
        scopeFull: 'Full context',
        allGroups: 'All groups',
        allGroupsHint: 'When off, only the groups below are screened (group-level opt-in).',
        groupIds: 'Group IDs',
        idsPlaceholder: 'Comma-separated IDs, e.g. 1, 2, 3',
        threshold: 'Escalate threshold',
        thresholdHint: 'Score that escalates to a block. Default 1.0: any High keyword, or 3+ distinct Medium keywords.',
        blockStatus: 'Block HTTP status',
        ...
        exemptionsHint: 'Matched subjects are capped to Max level (logged, never hard-blocked).',
        addExemption: 'Add exemption',
        userIds: 'User IDs',
        apiKeyIds: 'API Key IDs',
        maxLevel: 'Max level',
        save: 'Save Prompt Risk',
        saved: 'Prompt risk config saved',
        ...
        judge: {
          title: 'LLM Semantic Review',
          hint: 'When a keyword match would actually block, call an LLM once for a semantic verdict — telling "configuring my own VPN / authorized testing" apart from real abuse — to cut dual-use false positives. Fails open (allows) on any error.',
          enabled: 'Enable semantic review',
          baseUrl: 'Base URL (OpenAI-compatible)',
          model: 'Model',
          apiKey: 'API Key',
          apiKeyHint: 'Leave blank to keep the stored key; the plaintext is never returned on GET.',
          timeoutMs: 'Timeout (ms)',
          triggerLevels: 'Trigger levels',
          triggerLevelsHint: 'Only review requests that match these levels and would actually block (default: High only).',
          promptTemplate: 'Custom review prompt (system)',
          promptTemplatePlaceholder: 'Leave blank to use the built-in default prompt',
          recursionTitle: '⚠️ Prevent self-trigger (important)',
          recursionHint: 'The review request hits your own gateway and passes through this filter again; since it contains the original keywords, it can self-trigger or recurse. Use a dedicated API Key for the judge, add its api_key_id to an exemption below (max level = Low), and make sure that key\'s group is outside the content-moderation blocking scope.',
          addExemption: 'Add an exemption for the judge key',
          exemptionAdded: 'Exemption added (max = Low) — fill in the judge\'s dedicated api_key_id',
        },
      },
    },
    ...
    settings: {
      ...
      requestArchive: {
        ...
        captureResponse: 'Capture Response Usage',
        captureResponseHint: 'Records response size, hash, stream flag, and token usage without saving response content',
        dir: 'Archive Directory',
        dirHint: 'Enter an absolute path to move archives to another disk. Saving validates that the disk exists and the directory is creatable and writable; takes effect immediately without restart',
        dirDefault: 'Config default',
