...
- 仅 OpenAI OAuth 账号可配置 `codex_fingerprint_mode`；缺失、空值、`off` 或非法值都表示关闭收敛，保留客户端原始 device/session 标识。
- `device`、`session`、`full` 是显式 opt-in，创建、编辑和批量编辑提交时必须原样写入 `extra.codex_fingerprint_mode`；选择 `off` 时删除该键。
- passthrough 与普通 Responses 转发使用同一套派生 ID。每次账号 attempt 都要覆盖暂存指纹状态，不能在 failover 时复用上一账号的值。
...
- upstream billing probe 适用于全部 `type=apikey` 平台；单个/批量 probe 结果更新 `account.extra.upstream_billing_probe`，UI 根据采集时间显示 stale 状态。
- 创建 API Key 账号时 `upstream_billing_probe_enabled` 默认 `true`; 创建成功后等待首次 `probeUpstreamBilling(account.id)` 完成再刷新列表, probe 失败只告警而不撤销已创建账号。用户关闭 toggle 时必须显式提交 `false`; OAuth、Bedrock 等非 API Key 类型不发送该字段。
