...
| Item | Value |
|---|---|
| Integration branch | `feature/hy/10170_merge_upstream_v170` |
| Upstream remote / branch | `upstream` -> `https://github.com/Wei-Shaw/sub2api.git`; `main` |
...
| Semantic overlap review | 三方路径集合为 only-local 411、only-upstream 242、both 52。24 个后端网关和 22 个外围自动合并路径完成逐路径三方审查, 未发现可定位的自动合并缺陷；冲突修复复审确认 guard 顺序。 |
| Local features | 23 个合并前 tracked `docs/features/` 文件零删除。RequestArchive/RequestIntercept、Prompt Metrics/Risk 与 LLM judge、Token Analysis、组织用量、子管理员、OpenAI-compatible cache usage、默认 reasoning effort、用户并发 preset 和 quota flusher 均保留；功能重叠处采用上游实现。 |
| Upstream behavior | 合入分组利润控制、跨平台 API Key 上游账单探测与倍率同步、账号跨页全选/批量删除、内容审核代理、Responses/Gemini path allowlist、OAuth Responses namespace opt-in 摊平、compact home、订阅配额窗口到期对齐、proxy stream circuit 禁用开关和容器部署加固。新增 `backend/migrations/192_group_profit_control.sql` 与 `193_group_profit_control_auth_cache_invalidation.sql`。 |
...
| Item | Value |
|---|---|
| Integration branch | `feature/hy/10177_merge_sub2api_177` |
| Upstream remote / branch | `upstream` -> `https://github.com/Wei-Shaw/sub2api.git`; `main` |
...
## 2026-08-26 main sync (v0.1.183; awaiting review)

| Item | Value |
|---|---|
| Integration branch | `feature/hy/10183_merge_sub2api_183` |
| Upstream remote / branch | `upstream` -> `https://github.com/Wei-Shaw/sub2api.git`; `main` |
| Base before merge / first parent | `0747c896abf8202dff9935a30f020317ebdc91fc`（任务开始时最新本地 `main`） |
| Merge base | `baeac1f3de21d37b129405f092ef86c24b3f203d`（上游 v0.1.177） |
| Upstream head / second parent | `7634e3c23b5b9afc588c37b170820f63f1d41bbb` |
| Merge commit | **待用户审核，尚未创建；当前 `MERGE_HEAD=7634e3c23b5b9afc588c37b170820f63f1d41bbb`** |
| Upstream version / delta | 固定 SHA 的 `backend/cmd/server/VERSION` 为 `0.1.183`，不是用户口述的 0.1.178；该边界已包含 `0.1.178@49504adc` 至 `0.1.183@7634e3c2`。从 merge base 到固定上游共 410 commits、771 paths、`+71087/-5334`，不以标签、口述版本或更晚 upstream HEAD 替代 exact SHA。 |
| Conflict files | `.gitignore`; `backend/cmd/server/wire.go`; `backend/cmd/server/wire_gen.go`; `backend/cmd/server/wire_gen_test.go`; `backend/go.mod`; `backend/internal/config/config.go`; `backend/internal/pkg/apicompat/chatcompletions_responses_bridge.go`; `backend/internal/server/routes/gateway.go`; `backend/internal/service/openai_gateway_cc_pipeline.go`; `backend/internal/service/openai_gateway_chat_completions.go`; `backend/internal/service/openai_gateway_forward.go`; `backend/internal/service/openai_gateway_messages.go`; `backend/internal/service/openai_gateway_responses_chat_fallback.go`; `backend/internal/service/openai_responses_tool_schema.go`; `backend/internal/service/setting_service.go`; `backend/internal/service/settings_view.go`; `backend/internal/service/wire.go`; `frontend/src/api/admin/index.ts`; `frontend/src/components/account/CreateAccountModal.vue`; `frontend/src/components/admin/user/UserEditModal.vue` |
| Conflict handling | 20 个文本冲突按三方语义解决。Wire 源图同时保留本地 Token Analysis、RequestIntercept、组织用量等 provider，并接入上游 CN provider、Plugin、Model Plaza 生命周期后重新生成；Responses -> Chat 采用上游 reasoning cache 主实现，保留本地 `DropTemperature` / `DropMaxCompletionTokens` options；Tool Schema 采用上游有界 parser，继续清理本地 `text.format.schema` 的非支持 `format`；Responses 路由保持 subpath guard 位于 `RequestIntercept` 前。前端保留 OpenAI-compatible preset、`sub_admin` 权限和组织用量，并适配上游统一 `Select` 合同。未改写上游冲突外业务实现。 |
| Semantic overlap review | 对冲突和自动合并交叠路径按 Wire/config、网关/Responses、settings/repository、前端 API/组件分组审查。功能真正重叠处采用上游 reasoning cache、有界 schema parser、CN provider/Plugin/Model Plaza 实现；本地横切与独有能力继续保留。完整 Vitest 首轮发现两处合并适配问题：普通用户编辑触发无关权限目录 API、角色测试仍按旧原生 select 操作；仅修正合并合同并补跑全量验证，没有借机处理上游缺陷。 |
| Local features | 合并前 24 个 tracked `docs/features/` 文件全部保留、零删除；除本 ledger 追加外其余 feature 文档不改。RequestArchive/RequestIntercept、Prompt Metrics/Risk 与 LLM judge、Token Analysis、组织用量、子管理员、OpenAI-compatible cache usage、默认 reasoning effort、用户并发 preset、quota flusher 和 reasoning-only failover 均保留。 |
| Upstream behavior | 合入 Kimi/智谱/DeepSeek CN provider 与 adaptive protocol、Channel Monitor quota/quota_probe、OpenAI OAuth transport Plugin、正式 `/v1/responses/input_tokens`、Model Plaza 收敛、模型 Fast/Flex 与分时定价、Responses reasoning cache/tool identity、Tool Schema 资源上限、Go 1.27，以及 migrations 224-230。 |
| Upstream / baseline issue boundary | Windows 完整 default/integration/unit 测试中的 4 个 `TestPluginPackageInstaller` 用例因上游 Plugin installer 在临时目录 rename 时遇到文件锁而失败；unit 另有第一父既有 `/api/v1/auth/me` fixture 少 `admin_permissions:null` 的差异。两类问题均按“仅解决冲突，不修上游合并 bug”要求保留，不修改生产代码；排除 Plugin installer 用例后的 `internal/service` 全包通过。 |
...
| Semantic overlap review | 66 个双方修改路径按 Ent/Wire、网关/计费/认证、订阅幂等/管理、前端账号/路由/开关和 i18n 审查。自动合并发现三处必要适配：(1) 本地 `subscriptionHandlerService` 窄接口缺上游 `BulkSubscriptionAction` 签名，仅补签名；(2) 上游批量订阅 UI 对本地子管理员可见，把批量选择/工具栏/弹窗/命令 guard 收口为完整管理员（`authStore.isAdmin` 严格 `role === 'admin'`），保留后端未知写路由默认拒绝——后端 `CanAccessAdminRoute` 为默认拒绝白名单，`POST /subscriptions/bulk-action` 不在 sub_admin 规则内，本地 `reset-daily-filtered` 仍在；(3) 上游新增的 `SubscriptionsView.bulkActions.spec.ts` 未 mock `@/stores/auth` 也未装 Pinia，而合并后组件因本地 `useAuthStore` 需要它，故按本地既有范式（`SubscriptionsView.spec.ts`）补 `authState` + `vi.mock('@/stores/auth')`，其中 `isAdmin` 必须为 `true`：它同时决定批量工具栏是否渲染，以及 `loadGroups()` 走已 mock 的 `groups.getAll()` 而非未 mock 的 `subscriptions.searchGroups()`。此为合并接线，不属于修上游。其它独有本地行为不做大块 ours/theirs 覆盖。 |
...
| Upstream / baseline issue boundary | `TestOllamaProbeCallback_StaleLongDoesNotOverrideNewShort` 为 **flaky 而非确定性失败**：三次精确复跑结果为 1 通过 / 2 失败（stale long reset 通过 CAS）。测试将 `newShort` 设为 `now+5s`，而 `handle429` 已调度 gen@5s，CAS 结果取决于亚毫秒时序。`ratelimit_service.go`、`ratelimit_service_ollama_429.go` 与对应测试索引 blob 与固定上游完全一致 → 上游固有，本轮只记录等待上游修复；**按 flaky 跟踪**，避免其在 CI 静默转绿后掩盖后续回归。Go default 的 3 个 `backup_pg_dumper` 用例因 Windows 缺少 `sh.exe` 失败：依赖来自测试自身硬编码的 `exec.CommandContext(ctx, "sh", "-c", ...)`（5 处），生产 `backup_pg_dumper.go` 只调 `psql` 不经 shell，且该测试与实现文件 base/第一父/上游/index 四方 blob 一致，与本次合并无关（在 PATH 含 `sh.exe` 的 Git Bash 下 7 个用例全通过）。unit 另有第一父 `/auth/me` `admin_permissions:null` golden 差异：合并未触碰该 golden 块，本地独有 `dto.User.AdminPermissions` 无 `omitempty`，第一父即不匹配。前端完整 Vitest 失败自 12 降至 **8**：其中**批量订阅测试 4 个属于本次合并引入、已修**（详见下一行 Semantic overlap review）——该 spec 在目标上游自身为绿，上游 `SubscriptionsView.vue` 无 Pinia 依赖，仅在叠加本地 `useAuthStore` 后才失败，**不应归因为目标上游问题**；剩余 8 个为既有问题，均不修：Channel Monitor 旧 provider 数量断言 1 个（spec 与组件四方 blob 一致）、`GroupsView.codexManifest` 1 个（`GroupsView.vue` 的 `useAuthStore` 在 merge base 即存在）、`SubscriptionsView.userUsageLink` 6 个（本地版同样缺 `@/stores/auth` mock，第一父即红）。本机 `golangci-lint v2.9.0` 由 Go 1.26 构建，拒绝加载 Go 1.27 配置；`go mod tidy -diff` 只提示既有/目标 `go.sum` 的旧传递校验和与 `xxh3`，未改依赖元数据。按用户限定不修上游或第一父问题。 |
