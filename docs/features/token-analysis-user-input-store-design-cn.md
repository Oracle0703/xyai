...
**默认开启是有意决策**(负责人确认, codex 两轮审查均建议默认 0 已明确不采纳): 本 fork 为公司内部部署, "标准未定先存原料"是特性需求本身, 默认 0 等于默认关闭特性。面向通用部署的 fork 应自行评估改 0。

开启 `gateway.request_archive.enabled` 前运维需确认:

1. **保留期**: JSONL 清理周期已排定(全文已入库, 原文窗口仅供排障);
2. **DB 访问权限**: `token_analysis_user_inputs` 含员工输入全文(已脱敏), 管理端/DB 直连权限按敏感表对待;
3. **备份与清理**: DB 备份策略覆盖该表; 历史行清理(如按年龄删除)与质量评估回填周期不冲突;
4. **关闭方式**: `token_analysis.auto_index_interval_seconds: 0` 停自动索引, `input_store_max_chars: 0` 停全文留存(已索引行不受影响, 需手动清理)。
